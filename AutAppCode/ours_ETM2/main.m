%read: with SOD, with counter q, with augementation of the disturbance

function [td,jd,xid] = outputFeedback()
    rng('shuffle'); %reseed random number generator
    global tau_max delta q_max Ad Bd Hd Kd L1d L2d inv_H1d inv_H2d d dim_n dim_m dim_p%(Ad,Hd) observable; F1 and F2 are Hurwitz; eye(2)-expm(F2*delta)*expm(-F1*delta) is invertible
    tau_max = 5; %half of desired convergence instant
    delta = 2; %SOD of ETM on sensor-controller channel
    q_max = 6; %counter upper limit
...
%         det(H2d) %\neq 0
    ...
    hxd1_0 = [zeros(4,1);zeros(2,1)]; %xi(:,dim_n+dim_m+1:2dim_n+2dim_m)
    ...
    q_0 = 1; %xi(:,4dim_n+4dim_m+2)
    xid_0 = [xd_0;hxd1_0;hxd2_0;xcd_0;tau_0;q_0];
    ...
    plot(td,[xid(:,1) xid(:,dim_n+dim_m+1) xid(:,2*dim_n+2*dim_m+1) xid(:,3*dim_n+3*dim_m+1)])
    legend('x_1','hx1_1','hx2_1','x_s1')
    subplot(2,3,2) %2nd component of: state, obs1, obs2, pr
    plot(td,[xid(:,2) xid(:,dim_n+dim_m+2) xid(:,2*dim_n+2*dim_m+2) xid(:,3*dim_n+3*dim_m+2)])
    legend('x_2','hx1_2','hx2_2','x_s2')
    subplot(2,3,3) %3rd component of: state, obs1, obs2, pr
    plot(td,[xid(:,3) xid(:,dim_n+dim_m+3) xid(:,2*dim_n+2*dim_m+3) xid(:,3*dim_n+3*dim_m+3)])
    legend('x_3','hx1_3','hx2_3','x_s3')
    subplot(2,3,4) %4th component of: state, obs1, obs2, pr
    plot(td,[xid(:,4) xid(:,dim_n+dim_m+4) xid(:,2*dim_n+2*dim_m+4) xid(:,3*dim_n+3*dim_m+4)])
    legend('x_4','hx1_4','hx2_4','x_s4')
    subplot(2,3,5) %disturbance 1st component: real, obs1, obs2, pr
    plot(td,[xid(:,5) xid(:,dim_n+dim_m+5) xid(:,2*dim_n+2*dim_m+5) xid(:,3*dim_n+3*dim_m+5)])
    legend('d_1','hd1_1','hd2_1','d_c_1')
    subplot(2,3,6) %disturbance 1st component: real, obs1, obs2, pr
    plot(td,[xid(:,6) xid(:,dim_n+dim_m+6) xid(:,2*dim_n+2*dim_m+6) xid(:,3*dim_n+3*dim_m+6)])
    legend('d_2','hd1_2','hd2_2','d_c_2')
    ...
    for i=1:1:length(td)
        ...
        hxd2 = xid(i,2*dim_n+2*dim_m+1:3*dim_n+3*dim_m);
        xcd = xid(i,3*dim_n+3*dim_m+1:4*dim_n+4*dim_m);
        tau = xid(i,4*dim_n+4*dim_m+1);
        q = xid(i,4*dim_n+4*dim_m+2);
        
        out1(i) = sensor.D(tau,q); %observer event
        out2(i) = ETM.D(hxd1',hxd2',xcd'); %channel event
        out3(i) = sensor_ETM.D(tau,q); %observer event & channel event
    end
    out12 = out1 + out3; %observer event
    out23 = out2 + out3; %channel event
    subplot(2,1,1)
    plot(td,out12)
    legend('observer events')
    subplot(2,1,2)
    plot(td,out23)
    ...
    save data_ours_w_q td_w_q jd_w_q xid_w_q out23_w_q
end    
...
function dxid = F(xid)
    ...
    hxd2 = xid(2*dim_n+2*dim_m+1:3*dim_n+3*dim_m);
    xcd = xid(3*dim_n+3*dim_m+1:4*dim_n+4*dim_m);
...
%     u = 0; %null
    ...
    dxid = [dxd;dhxd1;dhxd2;hxc;dtau;dq];
end
...
function next_xid = G(xid)
    ...
    hxd2 = xid(2*dim_n+2*dim_m+1:3*dim_n+3*dim_m);
    xcd = xid(3*dim_n+3*dim_m+1:4*dim_n+4*dim_m);
    tau = xid(4*dim_n+4*dim_m+1);
    q = xid(4*dim_n+4*dim_m+2);
    
    out1 = sensor.D(tau,q); %observer event
    out2 = ETM.D(hxd1,hxd2,xcd); %channel event
    out3 = sensor_ETM.D(tau,q); %observer event & channel event
    
    [next_hxd1,next_hxd2] = sensor.dynamics_jump(hxd1,hxd2); %H1\xo[1]+H2\xo[2]
    next_xcd = predictor.dynamics_jump(hxd1,hxd2); %\xo[1]
    next_q = sensor_ETM.dynamics_jump(q); %mod(q,q_max)+1
    
    G1_xid = [xd;next_hxd1;next_hxd2;xcd;0;next_q];
    G2_xid = [xd;hxd1;hxd2;next_xcd;tau;q];
    G3_xid = [xd;next_hxd1;next_hxd2;next_hxd1;0;next_q];
    ...
    elseif (out1 == 1) && (out2 == 1) && (out3 == 0)
        ...
        index = round(rand(1))+2; %generate 2 or 3 randomly
        next_xid = [G2_xid G3_xid];
        next_xid = next_xid(:,index);
    end
end
...
function out = C(xid)
    ...
    out1 = sensor.C(tau,q); %observer event
    out2 = ETM.C(hxd1,hxd2,xcd); %channel event
    out3 = sensor_ETM.C(tau,q); %observer event & channel event
