...
close all
    ...
    global timer_max Ad Bd Hd Kd L1d d dim_n dim_m dim_p%(Ad,Hd) observable; F1 and F2 are Hurwitz; eye(2)-expm(F2*delta)*expm(-F1*delta) is invertible
...
%     tau_0 = 0; %xi(:,4dim_n+4dim_m+1)
    xid_0 = [xd_0;hxd1_0;xcd_0;timer_0];
...
%     out1 = zeros(size(td));
    out2 = zeros(size(td));
    for i=1:1:length(td)
        timer = xid(i,3*dim_n+3*dim_m+1);
%         tau = xid(i,4*dim_n+4*dim_m+1);
        
%         out1(i) = sensor.D(tau); %observer event
        out2(i) = ETM.D(timer); %channel event
    end
    plot(td,out2)
    legend('G2')
    ...
    save data_tt td_tt jd_tt xid_tt out2_tt
end    
...
function out = C(xid)
    ...
    hxd1 = xid(dim_n+dim_m+1:2*dim_n+2*dim_m);
    xcd = xid(2*dim_n+2*dim_m+1:3*dim_n+3*dim_m);
    timer = xid(3*dim_n+3*dim_m+1);
    
%     out1 = sensor.C(tau); %observer event
    out = ETM.C(timer); %channel event
