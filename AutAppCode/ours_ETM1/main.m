%read: with SOD, without counter q, with augementation of the disturbance

function [td,jd,xid] = outputFeedback()
clear all
close all
    rng('shuffle'); %reseed random number generator
    global tau_max delta Ad Bd Hd Kd L1d L2d inv_H1d inv_H2d d dim_n dim_m dim_p%(Ad,Hd) observable; F1 and F2 are Hurwitz; eye(2)-expm(F2*delta)*expm(-F1*delta) is invertible
...
%     legend('timer','events')
    ...
    save data_ours_wo_q td jd xid out2
end    
...
function next_xid = G(xid)
    ...
    G2_xid = [xd;hxd1;hxd2;next_xcd;tau];
